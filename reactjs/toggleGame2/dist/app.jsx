"use strict";
const Tile = (props) => <button className={`tile ${props.status ? 'blue' : 'red'}`} onClick={props.onClick}/>;
const BoardRow = (props) => <div className="row">
        {props.entries.map((value, index) => <Tile key={index} status={value} onClick={() => props.onClick(index)}/>)}
    </div>;
class Board extends React.Component {
    ...
    render() {
        return (<div>
                <BoardRow entries={this.state.row} onClick={(i) => {
                this.flipRowAndNeighbors(i);
            }}/>
            </div>);
    }
